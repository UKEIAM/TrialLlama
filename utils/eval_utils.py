...
def calculate_metrics(eval_output_path, gold_labels_file, ft_model_name):
    model_df = pd.read_csv(
        eval_output_path, header=None, names=["TOPIC", "NCT_ID", "LABEL"]
    )
    gold_df = pd.read_csv(
        gold_labels_file, header=None, names=["TOPIC", "NCT_ID", "LABEL"]
    )
    ...
    auc = roc_auc_score(merged_df["LABEL_gold"], merged_df["LABEL_pred"])
