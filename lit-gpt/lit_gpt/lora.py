...
class LoRAQKVLinear(LoRALinear):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0 and any(enable_lora):
            ...
            if enable_q:
                lora_ind.append(
                    torch.arange(0, self.in_features, device=self.weight.device)
                )
            if enable_k:
                lora_ind.append(
                    torch.arange(
                        self.in_features,
                        self.in_features + self.kv_embd_size,
    ...
    def zero_pad(self, x: torch.Tensor) -> torch.Tensor:
        ...
        result = result.index_copy(
            1, self.lora_ind, x.reshape(-1, shape)
        )  # (4096, 256)
        return result.view((*x.shape[:-1], self.out_features)).transpose(
            0, 1
        )  # (64, 64, 384)
    ...
    def forward(
        ...
        if self.rope_cache is None:
            self.rope_cache = self.build_rope_cache(
                idx
            )  # 2 * (block_size, head_size * rotary_percentage)
        # passing `attn_mask` to SDPA downgrades it to use the inefficient implementation. since we only need the mask
        # for the kv-cache support (only during inference), we only create it in that situation
        # this will be resolved by https://github.com/pytorch/pytorch/issues/96099
        if use_kv_cache and self.mask_cache is None:
            self.mask_cache = self.build_mask_cache(
                idx
